...
def visualise_mesh(networkx_graph, node_positions, ax, node_size=10, width=.3, linewidths=.5, node_color='k'):
    ...
    Parameters
    ----------
    networkx_graph : Python networkx object
        networkx format representation of the mesh connections betweeen superpixels. see mesh_statistics_tools.from_neighbor_list_to_graph() function.
    node_positions : numpy array
        n_superpixels x 2 array, (x,y) points of where to plot the individual superpixels/points.
...
def visualise_mesh_strain_ellipses(networkx_graph, node_positions, principal_strain, principal_angs, spixel_size, ax, mask=None, node_size=10, width=.3, linewidths=.5, node_color='k'):
    ...
    """ Plot on a matplotlib axis to visualize mesh strains as ellipses
