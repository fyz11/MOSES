...
def compute_vid_opt_flow(vidstack, params, mask=None, adaptive=False, ksize=3):
    """ Given a grayscale movie as a numpy vidstack, with the specified optical farneback params as params. Returns the stacked optical flow (frame by frame) for the entire video with optional masking.
    
    Set mask=None, adaptive=False for no masking.
    ...
    mask : numpy bool array
        optional n_rows x n_cols (single frame) boolean mask to only compute the optical flow in selected regions.
    adaptive : bool
        if mask is not given then specify if mask will be determined adaptively frame to frame using Otsu thresholding of pixel intensity.
    ksize : int
        size of disk kernel to dilate the mask if adaptive is set to True. 
    Output:
    -------
    flow : 
    ...
    for i in range(nframes-1):
        ...
        if mask is None and adaptive == False:
            flow.append(flow_frame[None,:])
        else:
            if mask is None and adaptive == True:
...
def compute_regular_spixels(imgsize, n_spixels=1000):
    ...
    ----------
    imgsize : tuple
        (n_rows x n_cols) tuple
    n_spixels : int
        the number of regular partitions of the image.
    
    Returns
    -------
    pos : numpy array
        mean positions of superpixels (n_superpixels x 2) numpy array
    mean_width_superpixel : int
        int specifying width of an average superpixel.
...
def count_point_density(poslist, spixelsmap, min_density=1):
    """ Counts the number of instances coordinates in poslist falls into each superpixel region specified by spixelsmap and determine superpixel regions whose region has less than the minimum required coordinate density.

    Parameters
    ----------
    poslist : numpy array 
        (n_points x 2) numpy array with channels in image coordinate (y,x) order.
    spixelsmap : numpy array
        the number of regular region of interst partitions (ROI) of the image. Each unique region is denoted with an integer from 0 to N-1 where N is the total number of regions.
    min_density : int 
        the minimum required number of points a region should contain. 

    Returns
    -------
    pos_density : numpy array 
        vector with number of points within each initial superpixel region.
    no_dense_regs : numpy array (vector of int)
        vector of each initial superpixel region which do not have the mindensity number of points within.

    """
    import numpy as np 
    
    poslist_ = poslist.astype(np.int)
    uniq_regs = np.unique(spixelsmap) # get unique areas.
    
    posregs = spixelsmap[poslist_[:,0], poslist_[:,1]] # get the region id the positions correspond to. 
    posdensity, binspos = np.histogram(posregs, range=[0, uniq_regs[-1]], bins=len(uniq_regs)) # accumulate the counts of points over the regions. (this is faster than list comprehension)
...
def dense_tracks2array_tracks(poslist):
    """ this function pads list of dense tracks to equal length tracks so all tracks can be concatenated and treated as a matrix.

    Default padding used is the starting position of the track, noting no removal of tracks.

    Parameters
    ----------
    poslist : list
        list of (y,x) coordinates of each superpixel at each frame.

    Returns
    -------
    trackarray : numpy array
        (n_superpixels, n_frames, 2) array in (y,x) image convention giving the final padded tracks.
...
def prop_spixels_frame_by_frame_forward( spixel_pos, optflow, spixel_size):
    ...
    Parameters
    -----------
    spixel_pos : numpy array
        (n_superpixels x 2) numpy array of superpixel centroids given as (y,x)-image coordinates
    optflow : numpy array
        (n_frames-1, n_rows, n_cols, 2) numpy array of frame-to-frame optical flow i.e. the output to above <compute_vid_opt_flow> function.
    spixel_size : float
        estimated mean superpixel width assuming they are square, output to above :py:meth:`compute_regular_spixels` function
    
    Returns
    -------
    meantracks : numpy array
        (n_superpixels, n_frames, 2) numpy array of the superpixel positions (centroids) in each frame
...
def prop_spixels_frame_by_frame_backward( spixel_pos, optflow, spixel_size):
    ...
    optflow : numpy array
        (n_frames-1, n_rows, n_cols, 2) numpy array of frame-to-frame optical flow i.e. the output to above <compute_vid_opt_flow> function.
    spixel_size : float
        estimated mean superpixel width assuming they are square, output to above <compute_regular_spixels> function.
...
def prop_spixels_frame_by_frame_forward_dense( spixel_pos, optflow, spixel_size, spixelsmap, mindensity=1):
    ...
    optflow : numpy array
        (n_frames-1, n_rows, n_cols, 2) numpy array of frame-to-frame optical flow i.e. the output to above <compute_vid_opt_flow> function.
    spixel_size : float
        estimated mean superpixel width assuming they are square, output to above :py:func:`compute_regular_spixels` function.
    
    Returns
    -------
    meantracks : 
        (n_points, n_frames, 2) numpy array
...
def prop_spixels_frame_by_frame_backward_dense( spixel_pos, optflow, spixel_size, spixelsmap, mindensity=1):
    ...
    -------
    meantracks : 
        (n_superpixels, n_frames, 2) numpy array
...
def compute_grayscale_vid_superpixel_tracks(vidstack, opt_flow_params, n_spixels=1000, mask=None, adaptive_mask=False, ksize_mask=3, direction='F', dense=False, mindensity=1):
    ...
    ----------
    vidstack : numpy array
        (n_frames x n_rows x n_cols) numpy array representing a gray-scale movie
    opt_flow_params : dict
        optical flow parameters for farneback optical flow implementation in opencv.
    n_spixels : int
        number of regular superpixels to decompose images.
    
    Returns
    -------
    optflow_ : numpy array
        (n_rows, n_cols, 2) optical flow displacements as (x,y) direction vectors.
    meantracks : numpy array
        (n_superpixels, n_frames, 2) numpy array of tracks.
...
def compute_grayscale_vid_superpixel_tracks_FB(vidstack, opt_flow_params, n_spixels=1000, mask=None, adaptive_mask=False, ksize_mask=3, dense=False, mindensity=1):
    """ Combines above functionalities into one easy to use function to compute the tracks forward and backward with optional masking
    ...
    optflow_ : numpy array
        (n_rows, n_cols, 2) optical flow displacements as (x,y) direction vectors.
    meantracks_F : numpy array
        (n_superpixels, n_frames, 2) numpy array of forward tracks.
    meantracks_B : numpy array
        (n_superpixels, n_frames, 2) numpy array of backward tracks. 
    ...
    return optflow_, meantracks_F, meantracks_B
